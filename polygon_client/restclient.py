...
class RestClient:
    ...
    def reset_raw_data_tables(engine, currency_pairs):
        with engine.begin() as conn:
            for curr in currency_pairs:
                ...
                conn.execute(text(
                    "CREATE TABLE IF NOT EXISTS " + curr[0] + curr[1] +
    ...
    # This creates a table for storing the (6 min interval) aggregated price data
    # for each currency pair in the SQLite database
    @staticmethod
            ...
            for curr in currency_pairs:
                ...
                try:
                    ...
                    if return_value >= upp_band and curr[
                        3].Prev_Action_was_Buy and return_value != 0:  # (return_value > 0) and (return_value_1 > 0) and
                ...
                try:
                    ...
                    if return_value <= loww_band and curr[
                        3].Prev_Action_was_Buy == False and return_value != 0:
                        # and  (return_value < 0) and (return_value_1 < 0)
                        curr[3].buy_curr(avg_price)
                except:
                    pass

    def calculate(self, currency_pairs):
